...
/**
 * 身份与权限拦截器（stats-service 版，同构 task-service）。
 *
 * <p>权限点不查本服务 DB，读共享 Redis 缓存 {@code auth:perms:{roleKey}}
 * （架构 4.1）；矩阵变更时 auth-user-service 主动失效缓存。</p>
 *
 * <p>M5 缺陷修复：缓存未命中时回源 auth-user-service 拉取该角色权限点
 * 并持久回填，不再因缓存过期误判 3001。</p>
 */
public class AuthInterceptor implements HandlerInterceptor {
    ...
    /** 白名单：健康检查 */
    private static final Set<String> WHITELIST = Set.of("/stats/api/v1/ping");

    private final RedisUtils redis;
    private final AuthUserClient authUserClient;

    public AuthInterceptor(RedisUtils redis, AuthUserClient authUserClient) {
        this.redis = redis;
    ...
    private String fetchAndCachePerms(String cacheKey, String roleKey) {
        try {
            Object data = authUserClient.getRolePermissions(roleKey).get("data");
            if (data instanceof Map<?, ?> map && map.get("permissions") instanceof Collection<?> perms) {
                ...
                redis.set(cacheKey, joined); // 与 auth 侧一致：持久保存，矩阵变更由 auth 主动失效
