...
- 任务管理（CRUD、状态流转、转派、进度、操作时间线、评论、附件、子任务）
- 任务列表与筛选（含 CSV 导出）
- 统计总览（KPI 与图表，多时间区间）
- 日程视图（月历）
- 权限管理（用户、角色、权限矩阵）
...
| 父任务 | 任务引用 | 系统生成 | 仅子任务有值，见 4.1.7 | 顶层任务为空 |
...
1. 任务详情抽屉内提供评论区，按时间正序展示（评论人、时间、内容）。
2. 对任务有可见性（3.4）即可评论；内容为纯文本，≤ 500 字符。
3. 评论不支持编辑；评论人可删除自己的评论，admin 可删除任何评论。
4. 新评论产生站内消息通知创建人与处理人（评论者本人除外），不发邮件。
5. 评论不写入操作时间线，独立存储；子任务同样支持评论。

#### 4.1.6 任务附件

1. 任务详情抽屉内提供附件区：上传、下载、删除。
2. 限制：单文件 ≤ 20MB，每任务 ≤ 10 个；类型白名单：pdf / doc / docx / xls / xlsx / ppt / pptx / txt / md / png / jpg / jpeg / zip / rar / 7z。
3. 权限：对任务有可见性即可上传与下载；删除限上传人本人或 admin。
4. 存储：服务端本地文件系统，存储路径可配置；数据库记录原始文件名、大小、上传人、上传时间；存储文件名使用 UUID，防止路径穿越；下载按原始文件名返回。
5. 一期不做病毒扫描与在线预览（见第 9 章）。
6. 子任务同样支持附件，附件归属所在任务，独立计数。

#### 4.1.7 子任务

1. 顶层任务下可创建子任务，**仅一级，不可再嵌套**。
2. 子任务为简化版任务：字段为标题、描述、优先级、处理人、到期时间、进度，编号独立（`TSK-`）；**任务类型继承父任务**，无来源渠道（固定为"网页"）。
3. 子任务状态机简化：**待办 → 进行中 → 已完成**，无验收环节，不支持归档；流转操作人与权限要求与主任务的受理、进度更新一致。
4. 父子约束：父任务存在未完成子任务时，**不可提交验收**（界面禁用并提示未完成子任务数）；子任务可在父任务进行中随时增删。
5. 创建权限：对父任务有可见性且具 `create` 权限点，创建人为操作者；处理人、创建人、admin 满足条件时均可创建。
6. 子任务的转派、调整优先级、调整到期、删除（限待办）规则与主任务一致；指派子任务同样触发 4.6.1 事件 1 的通知。
7. 展示与口径：任务列表平铺展示子任务（标题前加 `↳` 前缀并展示父任务编号，筛选正常生效）；统计总览与 KPI **只计顶层任务**（4.3 口径统一注明）；日程视图**包含子任务**，按其到期日聚合。
8. 父任务详情抽屉内展示子任务清单（编号、标题、状态、处理人、到期时间、进度），支持快捷创建与状态推进。
...
- 逾期任务的到期时间以红色标记。
- 子任务与顶层任务平铺展示，子任务标题前加 `↳` 前缀并展示父任务编号（规则见 4.1.7）。
...
以下指标均基于区间内且当前用户可见的任务集合计算，**只计顶层任务，不含子任务**。"未完成状态"指待办、进行中、待验收。
...
1. 月历网格，按任务的**到期日期**聚合展示（含子任务），每日格内列出当日到期的任务（含状态色点与标题，超出折叠为"+N"）。
...
| # | 事件 | 接收人 | 渠道 |
|---|---|---|---|
| 1 | 任务创建并指派（含子任务指派） | 处理人 | 站内 + 邮件 |
| 2 | 任务转派 | 新处理人 | 站内 + 邮件 |
| 3 | 提交验收 | 创建人 | 站内 + 邮件 |
| 4 | 验收通过 | 处理人 | 站内 + 邮件 |
| 5 | 验收驳回（含原因） | 处理人 | 站内 + 邮件 |
| 6 | 到期前 24 小时提醒（含子任务） | 处理人 | 站内 + 邮件，每任务仅一次 |
| 7 | 逾期提醒（含子任务） | 处理人、创建人 | 站内 + 邮件，每任务每日一次 |
| 8 | 新评论 | 创建人、处理人（评论者除外） | 仅站内 |
...
erDiagram
    ...
    TASK ||--o{ TASK_COMMENT : 评论
    TASK ||--o{ TASK_ATTACHMENT : 附件
...
| `task` | 任务 | 见 4.1.1 全部字段，另含父任务 id（顶层任务为空） |
| `task_timeline` | 操作时间线 | id、任务 id、操作人 id、动作、备注、时间 |
| `task_comment` | 任务评论 | id、任务 id、评论人 id、内容、时间 |
| `task_attachment` | 任务附件 | id、任务 id、原始文件名、存储文件名（UUID）、大小、上传人 id、时间 |
...
4. 任务的父任务引用只允许指向顶层任务（父任务引用为空的任务），保证子任务仅一级嵌套。
5. 时间线、审计日志只增不删（超过保留期的清理除外）。
...
| task | POST /api/tasks | 创建任务（body 含可选父任务 id，即创建子任务） | create |
| task | GET /api/tasks/{id} | 任务详情（含时间线、评论、附件、子任务清单、邮件记录） | 可见性规则 |
...
| task | GET /api/tasks/{id}/comments · POST /api/tasks/{id}/comments | 评论列表 / 发表评论 | 可见性规则 |
| task | DELETE /api/comments/{id} | 删除评论 | 评论人本人或 admin |
| task | POST /api/tasks/{id}/attachments | 上传附件 | 可见性规则 |
| task | GET /api/attachments/{id}/download | 下载附件 | 可见性规则 |
| task | DELETE /api/attachments/{id} | 删除附件 | 上传人本人或 admin |
...
| task.commented | 任务 id、评论人 | 通知创建人与处理人（站内） |
...
| 多级子任务嵌套 | 子任务仅一级，不可再嵌套 |
| 评论编辑与 @提醒 | 评论仅纯文本、可删不可改，无 @提及 |
| 附件在线预览与病毒扫描 | 附件仅上传/下载/删除；病毒扫描依赖企业终端安全体系，一期不内置 |
| 甘特图 / 看板视图 | 一期仅列表 + 月历 |
| 操作界面多语言 | 仅简体中文 |
...
| 评论 | 可见即可评论；仅评论人本人与 admin 可删除；新评论后创建人与处理人收到站内消息 |
| 附件 | 超限文件（大小/数量/类型）被拒绝并给出原因；下载文件与原始文件字节一致；非上传人且非 admin 删除返回 403 |
| 子任务 | 不可在子任务下再建子任务；父任务存在未完成子任务时提交验收被拒绝；列表中子任务带父任务编号；KPI 数值不含子任务 |
...
| 评论/附件/子任务 | 原型未覆盖，本 PRD 新增为内容域功能（4.1.5 至 4.1.7） |
