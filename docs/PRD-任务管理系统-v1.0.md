...
| 角色键 | 名称 | 定位 |
|---|---|---|
| `admin` | 系统管理员 | 系统配置与权限管理，不参与任务处理，不可被指派为处理人 |
| `taskAdmin` | 任务管理员 | 可见全部任务，管理任务全生命周期，可被指派为处理人 |
| `user` | 普通用户 | 仅可见与自己相关的任务，可被指派为处理人 |
...
默认矩阵即原型中的初始配置。普通用户默认不能创建任务，任务创建入口集中在管理员侧；如需放开，由系统管理员在权限矩阵中开启 `create`。
...
3. 权限矩阵的每次修改写入审计日志（操作人、时间、修改前后的矩阵差异），见 4.5.4。
...
| 验收通过 | 待验收 | 已完成 | 创建人 | editOwn |
| 验收驳回 | 待验收 | 进行中 | 创建人 | editOwn，驳回原因必填（≤ 500 字符） |
| 手动归档 | 已完成 | 已归档 | admin / taskAdmin | viewAll |
| 自动归档 | 已完成满 7 天 | 已归档 | 系统定时任务（每日执行一次） | 无 |
| 删除 | 待办 | （删除） | 创建人 | deleteOwn，需二次确认 |
...
| 可见性 | 普通用户登录后，任何接口均无法获取与其无关的任务（创建人、处理人均非本人） |
