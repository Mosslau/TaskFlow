...
按库分四组绘制全部 19 张表；**实线为库内 FK（数据库强制）**，跨库逻辑引用（无数据库约束，应用层保证）见 2.5 列表。属性仅列关键列，完整字段见第 3-6 章 DDL。
...
erDiagram
    ...
    API_KEY {
        bigint id PK
        bigint user_id FK "绑定的服务账号"
        text key_hash UK "Key 哈希"
        text status "active/disabled"
    }
```
...
-- OpenAPI 接入密钥（接口设计文档第 2 章；静态 Key，一期仅开放 POST /api/tasks）
CREATE TABLE api_key (
    id            BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    name          TEXT NOT NULL,                        -- 调用方名称，如 "运维监控系统"
    user_id       BIGINT NOT NULL REFERENCES app_user(id),  -- 绑定的专用服务账号（角色固定 taskAdmin）
    key_hash      TEXT NOT NULL UNIQUE,                 -- API Key 哈希（明文仅签发时展示一次，不落库）
    key_prefix    TEXT NOT NULL,                        -- 明文前 8 位（tfk_a1b2），列表展示与排查用
    status        TEXT NOT NULL DEFAULT 'active'
                  CHECK (status IN ('active', 'disabled')),
    last_used_at  TIMESTAMPTZ,                          -- 回源鉴权时刷新（分钟级精度）
    created_at    TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- 重新生成（regenerate）= 更新 key_hash 与 key_prefix，旧 Key 立即失效，无需新建行
```
...
| 项 | 说明 |
|---|---|
| 实体覆盖 | PRD 5.2 的 12 个实体全部落表；另按架构文档新增 `event_outbox`、两张 `processed_event`、三张 stats 聚合表；按接口设计文档新增 `api_key`（OpenAPI 接入密钥） |
