...
> 增补：附录 A `api_key` 表（2026-09-05，随接口设计评审引入，不影响已审定的 18 张表）
...
## 附录 A. api_key 表（增补，2026-09-05）

随接口设计评审引入（OpenAPI 静态 API Key 方案，见接口设计文档第 2 章），归属 `auth_user_db`：
...
CREATE TABLE api_key (
    ...
    status        TEXT NOT NULL DEFAULT 'active'
                  CHECK (status IN ('active', 'disabled')),
    last_used_at  TIMESTAMPTZ,                          -- 网关每次鉴权成功刷新
    created_at    TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
COMMENT ON COLUMN api_key.last_used_at IS '最近使用时间（网关鉴权成功时刷新）';
