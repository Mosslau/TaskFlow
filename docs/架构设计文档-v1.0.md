...
flowchart LR
    ...
    AU -->|③写缓存| Redis
    GW -->|④按绑定账号身份透传| TS[task-service<br/>POST /api/tasks]
```
...
"提交验收"是 PRD 4.1.2 状态机中**进行中 → 待验收**的流转（接口 `POST /api/tasks/{id}/submit-acceptance`）：处理人认为任务完成，提交后任务进入待验收，系统通知创建人验收。全链路如下：
...
sequenceDiagram
    ...
    U->>GW: POST /api/tasks/{id}/submit-acceptance
