...
| # | 差异 | 说明 |
|---|---|---|
| 1 | 入口加 Nginx | 裸部署下由 Nginx 托管前端静态资源并反向代理 `/{服务模块}/api/v1/**` 到 Gateway，同时终止 TLS（PRD 7.3.6 允许内网网关终止 TLS） |
...
- **迁移保护**：附件访问统一走 `GET /task/api/v1/attachments/{id}/download` 接口，不直接暴露文件路径；更换存储介质时仅改动 task-service 内部存储实现，前端与调用方无感。
...
| stats-service | 统计预聚合与查询 | `stats_db`：日粒度聚合表（按天 × 状态 / 优先级 / 处理人） | REST 接口（`GET /stats/api/v1/overview`）；消费事件增量聚合 |
...
**OpenAPI 调用（X-API-Key）的网关鉴权链路**：外部系统无登录流程，凭证为静态 API Key（方案见接口设计文档第 2 章，`api_key` 表归 auth-user-service 独占读写，网关不直连 auth_user_db，遵守 3.2 铁律 1）：

```mermaid
flowchart LR
    Ext[外部系统] -->|X-API-Key| GW[gateway-service]
    GW -->|①查缓存| Redis[(Redis<br/>key_hash → userId/roleKey/status)]
    Redis -->|未命中 ②回源| AU[auth-user-service<br/>查 api_key 表]
    AU -->|③写缓存| Redis
    GW -->|④按绑定账号身份透传| TS[task-service<br/>POST /task/api/v1/tasks]
```

1. 网关拿 Key 算哈希 → 先查 **Redis 缓存**（`key_hash → {userId, roleKey, status}`）；
2. 未命中则内部调 **auth-user-service** 鉴权回源，结果写 Redis；
3. 命中后网关把绑定服务账号的 `userId + roleKey` 透传给 task-service，后续权限点校验与 JWT 链路完全一致；
4. Key 停用 / 重生成时 auth-user-service **主动失效**对应缓存，与权限矩阵变更的缓存失效是同一套机制（见上第 4 步）；
5. `last_used_at` 由 auth-user-service 在回源鉴权时刷新（缓存命中时不逐次刷，分钟级精度足够排查用）。

该"网关 → Redis → auth-user-service 回源"模式与 JWT 黑名单检查同构，不引入新的链路类型。

### 4.2 任务状态流转链路（以"提交验收"为例）

"提交验收"是 PRD 4.1.2 状态机中**进行中 → 待验收**的流转（接口 `POST /task/api/v1/tasks/{id}/submit-acceptance`）：处理人认为任务完成，提交后任务进入待验收，系统通知创建人验收。全链路如下：

```mermaid
sequenceDiagram
    participant U as 处理人(浏览器)
    participant GW as gateway-service
    participant TS as task-service
    participant DB as task_db
    participant OUT as 投递线程
    participant MQ as RabbitMQ
    participant NS as notification-service

    U->>GW: POST /task/api/v1/tasks/{id}/submit-acceptance
    GW->>GW: 验签JWT + 查Redis黑名单
    GW->>TS: 转发(请求头带用户id+角色键)
    TS->>TS: ①查Redis: 该角色有无 updateAssigned 权限点
    TS->>DB: ②查任务: 状态=进行中? 处理人=当前用户?
    TS->>DB: ③查子任务: 有无未完成子任务?
    TS->>DB: ④事务: UPDATE任务状态=待验收<br/>+ INSERT时间线"提交验收"<br/>+ INSERT event_outbox事件
    TS-->>U: 200 OK (此刻状态已变)
    OUT->>DB: 轮询 event_outbox 未投递记录
    OUT->>MQ: 发布 task.acceptance.submitted
    MQ->>NS: 投递事件
    NS->>NS: 写站内消息(创建人) + 发邮件(创建人)
```

逐步说明：

1. **校验权限点（updateAssigned）**：Gateway 只做身份校验，权限点校验在 task-service 本地完成——从请求头取角色键，查 Redis「角色 → 权限点」映射，无 `updateAssigned` 直接 403。
2. **校验前置状态与操作人**：按 PRD 4.1.2 流转表，前置状态必须为进行中，操作人必须为该任务处理人；不满足返回明确业务错误（落实 PRD 验收标准"非法流转被服务端拒绝并返回明确错误"）。
3. **校验无未完成子任务**：PRD 4.1.7 父子约束，父任务存在未完成子任务时禁止提交验收，返回提示含未完成子任务数。
4. **单个数据库事务内做三件事**：

   ```sql
   BEGIN
     UPDATE task SET status='wait' WHERE id=?            -- 状态流转
     INSERT INTO task_timeline (...)                     -- 时间线"提交验收"（PRD 4.1.4 必录动作）
     INSERT INTO event_outbox (event_id, type, payload)  -- 事件记录（未投递状态）
   COMMIT
   ```

   三件事同生共死，不会出现"状态变了但时间线丢了"或"状态变了但事件丢了"。
5. **本地消息表（outbox）异步投递**：事务提交后，task-service 内后台线程轮询 `event_outbox` 中未投递记录，发布 `task.acceptance.submitted` 到 RabbitMQ，成功则标记已投递，失败下次轮询重投。不直接在业务事务里发 MQ，是为杜绝"消息已发出但事务回滚"的脏消息；该模式保证事件**至少送达一次（at-least-once）**，代价是可能重复，由消费端幂等兜底。
6. **notification-service 消费**：按 eventId 去重（幂等）→ 按 PRD 4.6.1 事件 3 生成站内消息 + 邮件给创建人；邮件失败指数退避重试 3 次。

关键设计点：

| 问题 | 结论 |
|---|---|
| 状态变更与通知同步还是异步？ | 状态变更**同步**（用户操作后立即看到"待验收"）；通知**异步**（秒级最终一致，PRD 8.4 允许） |
| 为什么不等邮件发完再返回？ | SMTP 可能慢或失败，不能阻塞用户操作；通知有独立重试，不影响业务状态 |
| 为什么用本地消息表而非直接发 MQ？ | 保证"状态变更"与"事件"的原子性，杜绝脏消息与丢消息 |
| 事件重复投递怎么办？ | 消费端按 eventId 幂等去重 |

PRD 4.1.2 中的其余流转（受理、验收通过、驳回、归档等）遵循同一链路模式：本地校验 → 单事务更新 + 时间线 + outbox → 异步投递事件 → notification / stats 消费。
